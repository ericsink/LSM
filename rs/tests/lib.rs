...
fn bson_simple() {
    fn f() -> lsm::Result<()> {
        ...
        bv.to_bson(&mut buf);
        println!("{:?}", buf);
        match try!(lsm::BsonValue::from_bson(&buf)) {
            lsm::BsonValue::BDocument(a) => {
                assert_eq!(3, a.len());

                let (ref k,ref v) = a[0];
                assert_eq!(k, "i32");
                match v {
                    &lsm::BsonValue::BInt32(n) => assert_eq!(n, 40),
                    _ => panic!(),
                }
                ...
                assert_eq!(k, "string");
                match v {
                    &lsm::BsonValue::BString(ref s) => assert_eq!(s, "forty"),
                    _ => panic!(),
                }
                ...
                assert_eq!(k, "array");
                match v {
                    &lsm::BsonValue::BArray(ref a) => assert_eq!(a.len(), 3),
                    _ => panic!(),
                }
